...
- Confirm in a deployed TrueForge run that a literal read-only tool name in `require_approval_for_tools` produces the expected pause. **Still open.** A running TrueForge 0.1.4 instance has been shown to register the control server and read all six tools with the annotations gating depends on (`evidence/trueforge-integration.txt`), but the pause itself needs a model-driven turn and has not been observed.
